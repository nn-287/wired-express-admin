...
class DeliverymanController extends Controller
{
    ...
    public function get_history_list(Request $request)
    {
       ...
       $paginator = Order::with(['delivery_address','customer'])->whereIn('order_status', ['delivered', 'returned', 'canceled'])
       ->where(['delivery_man_id' => $dm['id']])->latest()
    ...
    public function update_profile(Request $request)
    {
       ...
       $dm = DeliveryMan::where(['id' => $dm['id']])->first();
       if($request->phone != null){
           $dm->phone = $request->phone;
       }
       
       if ($request->password != null && strlen($request->password) > 5) {
            $dm->password = $request->phone;
        }
        $dm->save();
